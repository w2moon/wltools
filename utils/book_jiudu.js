...
var urlHead = "https://www.9dxs.com/2/2608/index.html";
...
function biqugeDownload(url, saveDir, cb) {
    ...
    request(url,{encoding:null,timeout:5000}, function (error, response, body) {
        ...
        if (response && response.statusCode == 200) {
            ...
            let $ = cheerio.load(txt);
            let nameStart = 10000;
            let num = $('li a','div.chapterlist').length;
            let total = num;
            let searchStarted = 0;
            ...
            $('li a','div.chapterlist').each((idx, elem) => {
                
                paged = true;
                let name = $(elem).text().replace(" ","_").replace("/","_");
                let page = urlHead.replace("index.html",$(elem).attr("href"));
                // name = name.replace("?","");
                // name = name.replace("*","");
                // name = name.replace("*","");
                // name = name.replace("**","");
